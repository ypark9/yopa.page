...
Current Node.js can load an environment file with `node --env-file=.env` where supported; `dotenv` remains a valid cross-version option. In either case, validate required values at startup, never commit secrets, and use the deployment platform secret store for credentials.
