...
IAM으로 잠긴 API Gateway를 Bruno(또는 Postman, Insomnia)로 두드려본 사람은 이 춤을 다 안다 — `aws configure export-credentials`, 값 세 개 복사, auth 패널에 붙여넣기, send, 두 시간 뒤 다시 send, 욕, 반복. SSO 쓰는 사람들 어딘가에 "이거 자동화하기"라는 TODO가 박혀 있다.
...
credential 세 필드는 빈 문자열. `profileName`이 비어있지 않으면 Bruno는 그 셋을 무시하고 AWS SDK의 profile chain으로 credential을 풀어낸다 — SSO 세션, role assume, `credential_process` 다 알아서 동작.
...
환경마다 파일 하나. 계정 전환은 이제 Bruno 환경 드롭다운 클릭 한 번 — auth 패널 건드릴 일 없음.
...
이 한 줄이 유일한 의식. SSO 토큰은 조직 설정에 따라 8–12시간. 아침에 한 번, 끝.
...
- **두 시간마다 재export.** SSO 임시 cred는 짧다. 정적 복붙은 계속 새로 갱신해야 한다. `profileName`이면 SDK가 알아서 갱신.
- **환경 헷갈리기.** auth 블록은 dev/qa/prod 다 똑같고 환경 변수만 바뀐다. 프로필별 access key가 엉뚱한 패널에 박혀있는 일 없음.

## 실제로 밟은 함정들

- **SSO 토큰 만료.** Bruno가 stale 세션으로 sign 시도하고, 헷갈리는 403이 돌아온다. `aws sso login --profile <name>` 하고 재시도. 자주 일어나면 짧은 alias 만들어두기.
...
- **Auth mode 혼용.** `accessKeyId`에 실수로 진짜 access key 남겨두면 버전에 따라 그걸 우선 쓸 수 있다. credential 세 필드는 무조건 빈 문자열로 유지.
- **`credential_process` 프로필도 OK.** 회사가 `credential_process`(corporate SSO wrapper, Granted 등) 쓰면 Bruno가 그것도 알아서 picks up — 같은 `profileName` 필드, 추가 설정 X.
...
Credential 세 필드는 빈 문자열, `profileName: "{{aws_profile}}"`, 환경마다 env var 하나, 아침에 `aws sso login`. API client에 임시 cred 복붙 그만.
