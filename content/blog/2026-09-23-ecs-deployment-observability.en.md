...
- **A green timeline is not a healthy service.** The timeline ends at "deployment complete." Whether the new version *works* is still a CloudWatch question, and for an agent gateway that means watching the actual task behaviour, not the rollout state.
