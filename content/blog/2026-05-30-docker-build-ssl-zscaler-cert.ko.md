...
author: Yoonsoo Park
description: "Zscaler(혹은 비슷한 TLS intercepting proxy) 환경에서 Docker build 중 `pip install`이 cert verification 에러로 죽는다면, 내가 쓰는 두 가지 주입 패턴 — 직접 만든 이미지용과 Lambda 번들링용 — 그리고 둘 다에서 한 번씩 밟은 함정들."
categories:
  - AWS
  - Docker
tags:
...
회사 네트워크가 Zscaler — 또는 Netskope, Cisco Umbrella, 아무튼 TLS interception 하는 "secure web gateway" — 뒤에 있다면, `docker build` 도중 이런 에러를 본 적 있을 것이다:
...
호스트 노트북은 멀쩡하다 — macOS Keychain이 프록시 root CA를 신뢰하니까. 컨테이너는 신뢰하지 않는다. 그냥 깨끗한 Linux filesystem에 public CA bundle만 들어 있을 뿐이다. 프록시는 PyPI로 가는 TLS handshake를 가로채서 자기 cert를 들이미는데, 컨테이너는 이게 누가 서명한 건지 모른다.

해결책은 양쪽 다 같은 아이디어다: **패키지 매니저가 돌기 전에 프록시 root CA를 컨테이너 trust store에 넣어라**. 다만 Dockerfile을 내가 쓸 수 있느냐에 따라 방법이 갈린다.

## Pattern 1 — Dockerfile을 내가 쓸 수 있는 경우 (custom image, `DockerImageAsset`)

쉬운 케이스. base64 인코딩된 cert를 build arg로 받아서 설치하는 `RUN` 블록을 일찍 넣어 둔다.
...
두 가지가 중요하다:
- **위치**. 이 블록은 `pip install`, `apt-get update`(HTTPS 거치는), `npm install` 등 인터넷 타는 모든 명령 *앞*에 와야 한다.
- **build arg로 게이팅한다**. arg가 비어 있으면 (CI/CD처럼 프록시가 없는 환경) 블록은 no-op. 프로덕션 이미지에 Zscaler cert를 굽고 싶진 않다.
...
## Pattern 2 — Dockerfile을 내가 못 쓰는 경우 (`PythonFunction`, `PythonLayerVersion`)

이게 처음에 몇 시간 헤맸던 케이스다. AWS CDK의 `@aws-cdk/aws-lambda-python-alpha`(`PythonFunction`, `PythonLayerVersion`)는 Lambda 코드를 번들링할 때 `public.ecr.aws/sam/build-python3.13` (또는 해당 런타임 이미지)을 띄워서 *그 안에서* `pip install`을 돌린다. 그 Dockerfile은 내가 쓰는 게 아니다. Pattern 1이 안 통한다.

해결: Zscaler cert를 미리 박아둔 커스텀 번들링 이미지를 만들어 ECR에 푸시하고, CDK에 기본 SAM 이미지 대신 그걸 쓰라고 알려준다.
...
CDK 쪽에서는 devMode일 때만 번들링 이미지를 갈아끼운다:
...
처음 이 문제 부딪혔는데 내가 쓴 custom image라면 Pattern 1 — Dockerfile 다섯 줄에 build arg 하나. `PythonFunction`/`PythonLayerVersion`(혹은 번들링 이미지를 알아서 골라 쓰는 CDK construct) 쪽이라면, 한 번 마음먹고 커스텀 번들링 이미지 만들어서 ECR에 푸시하고 `devMode` 체크 뒤에 숨겨라.

Pattern 2 셋업하는 데 들이는 10분이, 다음에 만날 "cert 분명 추가했는데 왜 `pip`이 또 죽지" 세 시간을 살려준다 — 왜냐하면 그 `pip`은 내가 쓴 컨테이너가 아니라 CDK가 골라서 띄운 컨테이너 안에서 돌고 있고, 그 컨테이너를 owning 하지 않으면 cert를 박을 방법이 없기 때문이다.
