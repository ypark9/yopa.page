...
Our platform is 100% AWS-native. The primary execution environment is **Amazon Bedrock AgentCore**, which is basically a managed runtime for AI agents with built-in memory, tool routing, and IAM-based auth. We needed to build agentic workflows that could handle dozens of backend actions (knowledge base search, document extraction, custom AI actions, etc.) for financial domain customers.
...
AgentCore Runtimes have a **strict 30-second initialization limit**. That's not configurable. LangGraph depends on `langchain-core`, which brings in `pydantic-core` (Rust-based C-extensions), and depending on your integrations, you may also pull in `numpy` and other heavy packages. These all need pre-compiled binaries for Linux ARM64. When you try to `pip install` them during cold start, you reliably blow past the 30-second limit.
...
**Cold starts?** Under 500ms. We use UV bytecode compilation (`UV_COMPILE_BYTECODE=1` in our Dockerfile) which pre-compiles Python files during the Docker build. No heavy C-extension compilation overhead. CDK deploys directly without Docker pre-compilation tricks.
...
~16 lines. In LangGraph, the same tool needs a state TypedDict, a node function, graph wiring, and compilation, which adds up to roughly 40-50 lines. Multiply that by 15 tools and the difference is significant.
...
This pattern was born from a real security review, not from upfront design. The LLM was occasionally hallucinating tenant IDs in early testing. It just made up tenant IDs that looked plausible. In a financial platform, that's a nightmare. Wrappers killed that problem entirely.
...
During the early PoC, this felt like a minor issue because we mostly exposed sync tools first. But looking at our actual backend, roughly half of our actions are asynchronous. Document processing via Textract, async LLM inference, batch data operations... these are all long-running jobs that return `pending` and complete later.
