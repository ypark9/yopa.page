...
author: Yoonsoo Park
description: "AWS에서 벡터 스토어를 고르는 건 이제 다섯 개 중 하나를 고르는 문제가 아니다. OpenSearch, pgvector, Neptune Analytics, Bedrock Knowledge Bases, Kendra에 S3 Vectors가 합류하면서, 진짜 질문은 '내 워크로드가 비용/지연 곡선의 어느 구간에 사는가'가 됐다. 업데이트된 결정 프레임워크를 정리한다."
categories:
  - AWS
  - Machine Learning
  - RAG
...
> 예전엔 AWS에서 벡터 스토어를 고르는 게 다섯 개 중 하나를 고르는 문제였다. **Amazon S3 Vectors**가 등장하면서 이제는 *내 워크로드가 비용/지연 곡선의 어느 구간(tier)에 속하는가*로 다시 묻는 게 맞다. 각 옵션이 뭘 잘하고, 언제 이기고, 경쟁 대신 어떻게 점점 더 조합되는지 — 업데이트된 결정 프레임워크다.
...
## 멘탈 모델: 미인대회가 아니라 비용/지연 tier

대부분의 "어떤 벡터 DB 써야 하나" 가이드는 서비스를 나란히 세워놓고 하나 골라 올인하라는 식이다. 현실에서 AWS 네이티브 옵션들은 이제 **비용 vs 지연 곡선** 위에 깔끔하게 정렬된다:
...
2025년 이후 흥미로운 변화는 이 표의 맨 위와 맨 아래가 이제 **계층적으로 묶인다(tier together)**는 것이다. cold 데이터에 메모리 가격을 내는 대신, hot 벡터는 OpenSearch에, 긴 꼬리는 S3 Vectors로 흘려보낸다. 아래에서 더 다룬다.

## Amazon S3 Vectors — 비용 바닥 (핵심 추가분)
...
**적합한 자리:** 드물게 또는 배치로 쿼리되는 크고 오래 사는 벡터셋 — 에이전트 장기 메모리, 아카이브성 시맨틱 검색, "지금까지 적재한 모든 것 검색" 코퍼스. 고QPS 실시간 서빙에는 명시적으로 *부적합*하다.
...
## Amazon OpenSearch Service — 규모와 지연의 챔피언

OpenSearch는 아주 많은 벡터를 대상으로 **sub-second(흔히 한 자릿수 ms) 지연에서 높은 QPS**가 필요할 때의 답으로 남아 있다. 운영 유연성이 가장 크고 billion-scale에서 가장 검증됐다.
...
## Aurora / RDS PostgreSQL + pgvector — 통합의 달인

벡터가 관계형 데이터 옆에 산다면 `pgvector`가 가장 저항이 적은 길이다: 한 엔진에서 벡터 검색과 ACID 트랜잭션을, 익숙한 도구로.
...
**트레이드오프:** HNSW 인덱스는 메모리에 살고 싶어 해서 수백만 벡터를 넘기면 인스턴스 사이징이 결정적이 된다. 피크 벡터 처리량보다 "관계형 + 벡터를 한 트랜잭션 스토어에" 두는 가치가 클 때 최적.

## Amazon Bedrock Knowledge Bases — RAG로 가는 가장 빠른 길

실제로 원하는 게 운영할 벡터 DB가 아니라 *retrieval-augmented generation* 그 자체라면, Bedrock Knowledge Bases가 매니지드 정문이다: 적재 → 청킹 → 임베딩 → 검색 → (선택) 생성, 벡터 스토어는 추상화돼 보이지 않는다.
...
**트레이드오프:** 검색 내부를 덜 통제한다. 핵심은 마이그레이션 경로다 — Bedrock KB로 검증을 시작하고, KB가 못 채우는 구체적 니즈를 입증했을 때만 자체 관리 스토어로 내려가라.

## Amazon Neptune Analytics — 관계 전문가

Neptune Analytics는 벡터 유사도와 **그래프 순회**를 결합해 GraphRAG를 구동한다: 단순 유사도 검색으로는 불가능한 multi-hop, 관계 인지 검색. 지식 그래프와 촘촘히 연결된 문서에서는, 단순 최근접 이웃이 아니라 *연결*에 의존하는 질문의 답 품질을 실질적으로 끌어올린다.

**트레이드오프:** 순수 벡터 검색보다 모델링 복잡도가 높고, 데이터에 진짜로 풍부한 관계가 있을 때만 값을 한다. "비슷한 청크 찾기"로 충분하면 과한 선택이다.

## Amazon Kendra GenAI Index — 턴키 엔터프라이즈 검색
...
**트레이드오프:** 가장 덜 유연하고 소규모에서 가장 싸지도 않지만, "누가 무엇을 볼 수 있는지를 존중하며 내부 문서를 검색한다"는 요구에는 가장 빠른 가치 실현.

## 결정 단축키

- **고QPS, 실시간, 대규모** → OpenSearch (cold 벡터는 S3 Vectors로 흘려보냄).
- **수십억 벡터, 드물거나 배치성 쿼리, 비용이 제약** → S3 Vectors.
- **관계형 데이터 옆의 벡터, ACID 필요** → Aurora/RDS pgvector.
- **이번 주 안에 RAG 출시, 운영할 DB는 사양** → Bedrock Knowledge Bases.
- **답이 유사도가 아니라 관계에 달림** → Neptune Analytics (GraphRAG).
- **접근 제어 있는 엔터프라이즈 문서 검색, ML 팀 없음** → Kendra GenAI Index.

## 피해야 할 안티패턴

- **전체 코퍼스를 hot 스토어 하나에 맞춰 사이징.** 2025년엔 마땅한 cold tier가 없어서 이게 디폴트였다. S3 Vectors가 생긴 지금, 거의 안 쓰는 벡터에 인메모리 가격을 내는 건 필연이 아니라 선택이다 — tier로 나눠라.
- **첫 배포부터 과도한 엔지니어링.** Bedrock Knowledge Bases로 동작하는 RAG 데모가 나온다면 거기서 시작하고, 구체적 한계를 입증했을 때만 마이그레이션하라.
- **모든 데이터 타입을 한 서비스에 욱여넣기.** 하이브리드가 정상이다: hot은 OpenSearch, cold 대량은 S3 Vectors, 관계는 Neptune, 구조화 데이터는 pgvector. 도그마가 아니라 데이터 형태별로 골라라.
